...
@push('styles')
<style>
/* === DESIGN TOKENS — Blue/Gold theme for SMP === */
:root {
    --gold:          #C9A84C;
    --gold-light:    #E8C97A;
    --gold-dark:     #A07830;
    --gold-pale:     #F5EDD8;
    --forest:        #0B2545; /* Dark Navy for background */
    --forest-mid:    #134074; /* Mid Navy */
    --forest-soft:   #1D2D44; /* Slate Blue */
    --moss:          #2563eb; /* Royal Blue */
    --moss-light:    #3b82f6; /* Sky Blue */
    --ivory:         #FAF7F0;
    --ivory-dark:    #EDE8DC;
    --text-dark:     #0F172A;
    --text-mid:      #334155;
...
.info-card-header.bg-docs i { color: var(--moss); }
...
.doc-icon-wrapper.image   { background: var(--success-bg); color: var(--success);   }
...
.action-btn.btn-verify {
    background: linear-gradient(135deg, var(--moss-light), var(--forest-mid));
    color: white;
...
<div class="detail-container">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
            <div>
                ...
                <div class="meta-info">
                    ...
                    <i class="fas fa-layer-group me-1"></i>{{ $applicant->school_program }}
                    <span class="mx-2">•</span>
                    <i class="fas fa-calendar me-1"></i>{{ $applicant->registered_at_label }}
                </div>
            </div>
            <div class="header-actions">
                <a href="{{ route('admin.smp.dashboard') }}" class="btn-header btn-outline">
                    <i class="fas fa-arrow-left"></i>Kembali
                </a>
                <a href="{{ route('admin.smp.print', $applicant->id) }}" class="btn-header btn-gold" target="_blank">
            ...
            <div class="info-card-body">
                <div class="data-grid">
                    ...
                    <div class="data-item">
                        ...
                        <span class="data-value">{{ $applicant->citizenship ?? 'WNI' }}</span>
                    </div>
                    <div class="data-item">
                    ...
                    <div class="data-item">
                        ...
                        <span class="data-value">{{ $applicant->siblings_count ?? '0' }}</span>
                    </div>
                    <div class="data-item">
                        <span class="data-label">Anak Ke-</span>
                        <span class="data-value">{{ $applicant->child_order ?? '1' }}</span>
                    </div>
                    <div class="data-item full-width">
                        <span class="data-label">Berkebutuhan Khusus</span>
                        <span class="data-value">{{ $applicant->disability ?? 'Tidak Ada' }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col-md-6">
                <div class="parent-card">
                    ...
                    <div class="parent-info">
                        ...
                        Berkebutuhan Khusus: {{ $applicant->father_disability ?? 'Tidak Ada' }}
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="parent-card">
                    ...
                    <div class="parent-info">
                        ...
                        Berkebutuhan Khusus: {{ $applicant->mother_disability ?? 'Tidak Ada' }}
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="parent-card" style="border-left: 3px solid var(--info);">
                    ...
                    <div class="parent-info">
                        ...
                        Berkebutuhan Khusus: {{ $applicant->guardian_disability ?? 'Tidak Ada' }}
                    </div>
                </div>
            </div>
        </div>
                ...
                @if(collect($documents)->filter(fn($d) => $d['path'])->count())
                    <div class="docs-grid">
                        @foreach($documents as $type => $doc)
                            @php
                                $previewUrl = $doc['path'] ? route('admin.smp.document.preview', [$type, $applicant->id]) : null;
            ...
            <div class="col-lg-4">
                <div class="action-panel">
                    ...
                    <div class="action-grid">
                        <a href="{{ route('admin.smp.edit', $applicant->id) }}" class="action-btn btn-edit">
                            <i class="fas fa-edit"></i>Edit Data
                        </a>

                        <form action="{{ route('admin.smp.delete', $applicant->id) }}" method="POST"
                        ...
                        <form action="{{ route('admin.smp.status.update', $applicant->id) }}" method="POST" class="status-change-card"
                        ...
                        <a href="{{ route('admin.smp.print', $applicant->id) }}" class="action-btn btn-print full-width" target="_blank">
    ...
    <div id="tab-activity" class="tab-content">
        @include('admin.partials.applicant-activities', [
            ...
            'activityStoreRoute' => route('admin.smp.activities.store', $applicant->id),
        ])
    </div>
